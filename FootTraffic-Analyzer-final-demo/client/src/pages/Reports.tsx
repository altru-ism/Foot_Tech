...
const Reports: React.FC = () => {
  ...
  // New export filters
  const [footTrafficMin, setFootTrafficMin] = useState<number | ''>('');
  const [footTrafficMax, setFootTrafficMax] = useState<number | ''>('');
  const [dwellMin, setDwellMin] = useState<number | ''>(''); // hours
  const [dwellMax, setDwellMax] = useState<number | ''>(''); // hours
  ...
  const handleExport = async () => {
    ...
    let filteredReports = selectedLocations.includes('all') 
      ? barangayReports 
    ...
    // Apply numeric filters: average foot traffic (people) and total dwell time (hours)
    filteredReports = filteredReports.filter(report => {
      const withinTrafficMin = footTrafficMin === '' || report.avgFootTraffic >= (footTrafficMin as number);
      const withinTrafficMax = footTrafficMax === '' || report.avgFootTraffic <= (footTrafficMax as number);

      // Parse total dwell time like "X hrs Y min" into decimal hours
      let dwellHours = 0;
      const totalMatch = /(\d+)\s*hrs?\s*(\d+)?\s*min/i.exec(report.totalDwellTime);
      if (totalMatch) {
        const hrs = parseInt(totalMatch[1] || '0');
        const mins = parseInt(totalMatch[2] || '0');
        dwellHours = hrs + mins / 60;
      }
      const withinDwellMin = dwellMin === '' || dwellHours >= (dwellMin as number);
      const withinDwellMax = dwellMax === '' || dwellHours <= (dwellMax as number);
      return withinTrafficMin && withinTrafficMax && withinDwellMin && withinDwellMax;
    });

    if (filteredReports.length === 0) {
      toast({
        title: 'No Results',
        description: 'No reports match the selected filters. Adjust the ranges and try again.',
        variant: 'destructive'
      });
      return;
    }
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="border rounded-md p-3">
                  <p className="text-xs text-gray-500 mb-2">Foot Traffic Range (avg people)</p>
                  <div className="flex items-center gap-2">
                    <Input
                      type="number"
                      placeholder="Min"
                      value={footTrafficMin}
                      onChange={(e) => setFootTrafficMin(e.target.value === '' ? '' : parseInt(e.target.value))}
                    />
                    <span className="text-xs text-gray-500">to</span>
                    <Input
                      type="number"
                      placeholder="Max"
                      value={footTrafficMax}
                      onChange={(e) => setFootTrafficMax(e.target.value === '' ? '' : parseInt(e.target.value))}
                    />
                  </div>
                </div>
                <div className="border rounded-md p-3">
                  <p className="text-xs text-gray-500 mb-2">Total Dwell Time Range (hours)</p>
                  <div className="flex items-center gap-2">
                    <Input
                      type="number"
                      placeholder="Min hrs"
                      value={dwellMin}
                      onChange={(e) => setDwellMin(e.target.value === '' ? '' : parseInt(e.target.value))}
                    />
                    <span className="text-xs text-gray-500">to</span>
                    <Input
                      type="number"
                      placeholder="Max hrs"
                      value={dwellMax}
                      onChange={(e) => setDwellMax(e.target.value === '' ? '' : parseInt(e.target.value))}
                    />
                  </div>
                </div>
              </div>
