...
# Apply consistent CORS headers to all responses
@app.after_request
...
class StatsExporter:
    def __init__(self, location="Divisoria", filename="tracking_statistics.json"):
        ...
        if not os.path.exists(self.filename):
            ...
            # Robustly handle possibly corrupted or NDJSON stats files
            try:
                ...
                if not content:
                    ...
                    try:
                        ...
                        self.all_stats = json.loads(content)
                        ...
                        for line in content.splitlines():
                            ...
                            try:
                                ...
                                if isinstance(obj, dict):
                                    # If line is already a records container
                                    if "records" in obj and isinstance(obj["records"], list):
                                        parsed_records.extend(obj["records"])
                                    else:
                                        parsed_records.append(obj)
                            except Exception:
                                # Skip malformed line
                                continue
                        self.all_stats = {"records": parsed_records}
                        # Rewrite the file in canonical JSON format to repair it
                        self._save_stats()
            except Exception:
                # As a last resort, reset to a clean structure
                self.all_stats = {"records": []}
                self._save_stats()
...
def generate_frames():
    ...
    while stream_active:
        try:
            # Use FFMPEG backend for HLS streams to improve stability
            if str(video_path).lower().endswith('.m3u8') or 'm3u8' in str(video_path).lower():
            ...
            while stream_active:
                ...
                if not success:
                    ...
                    if consecutive_failures >= max_consecutive_failures:
                        if "youtube" in str(video_path).lower() or (str(video_path).lower().endswith('.m3u8') or 'm3u8' in str(video_path).lower()):
    ...
    try:
        ...
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            try:
                ...
                # Prefer progressive, non-HLS formats (both audio and video, protocol https)
                progressive = [
                    f for f in formats
                    ...
                    and 'm3u8' not in str(f.get('ext', '')).lower()
                ]
                ...
                if progressive:
                    ...
                    selected_format = progressive[-1]
                    video_url = selected_format['url']
                    logger.info(f"Using progressive URL, format {selected_format.get('format_id')} {selected_format.get('height')}p")
                else:
                    # Fallback to HLS if live stream only provides m3u8
                    hls_formats = [
                        f for f in formats
                        if 'm3u8' in str(f.get('protocol', '')).lower() or 'm3u8' in str(f.get('ext', '')).lower()
                    ]
                    if not hls_formats and 'url' in info and ('m3u8' in str(info.get('url', '')).lower()):
                        hls_formats = [{'url': info['url'], 'format_id': info.get('format_id', 'unknown')}]

                    if hls_formats:
                        # pick the highest height or tbr
                        def hls_key(f):
                            return (f.get('height') or 0, f.get('tbr') or 0)
                        hls_formats.sort(key=hls_key)
                        selected_format = hls_formats[-1]
                        video_url = selected_format['url']
                        logger.info(f"Using HLS URL (m3u8), format {selected_format.get('format_id', 'm3u8')}")
                    else:
                        raise Exception("No suitable video format found")

                # Verify the URL is accessible
                logger.info("Verifying video URL accessibility...")
                # Use FFMPEG for HLS URLs during verification
...
def process_youtube():
    ...
    try:
        ...
        try:
            ...
            # Verify stream is accessible (use FFMPEG for HLS)
            if 'm3u8' in video_url.lower():
                cap = cv2.VideoCapture(video_url, cv2.CAP_FFMPEG)
            else:
                cap = cv2.VideoCapture(video_url)
